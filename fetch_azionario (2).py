...
def compute_indicators(df: pd.DataFrame) -> dict | None:
    ...
    ultimo_pivot_h_per_barra = [None] * len(high_list)
    _ultimo_h = None
    _pivot_iter = iter(zigzag_pivots)
    _prossimo = next(_pivot_iter, None)
    for idx in range(len(high_list)):
...
def process_batch(batch):
    """Scarica e calcola gli indicatori per un batch di titoli. Ritorna (righe_ok,
    ticker_falliti). Scrive anche i file chart/regole per ogni titolo riuscito.
    Estratta a parte per poter essere richiamata due volte: una per l'universo
    normale, una per i tentativi di fallback (es. borsa alternativa)."""
    rows_ok = []
    failed = []
    ...
    for u in batch:
        ...
        rows_ok.append((row, t, chart, u["nome"], summary))
...
def main():
    ...
    def salva_riusciti(rows_ok):
        for row, t, chart, nome, summary in rows_ok:
            results.append(row)
    ...
    for batch in chunked(universe, BATCH_SIZE):
        ...
        salva_riusciti(rows_ok)
        errors.extend(failed)

    # ---- Fallback borsa alternativa: solo per il basket iShares, .MI -> .DE ----
    # (i ticker di questo basket vengono da un CSV senza borsa esplicita: proviamo
    # prima .MI, e per chi fallisce ritentiamo su Xetra prima di arrenderci)
    ticker_falliti_ishares = [
        u for u in universe
        if u["ticker"] in errors and u["regione"] == "ISHARES" and u["ticker"].endswith(".MI")
    ]
    if ticker_falliti_ishares:
        print(f"Fallback .MI->.DE per {len(ticker_falliti_ishares)} ticker iShares falliti...")
        fallback_batch = [
            {**u, "ticker": u["ticker"][:-3] + ".DE"} for u in ticker_falliti_ishares
        ]
        for batch in chunked(fallback_batch, BATCH_SIZE):
            rows_ok, failed = process_batch(batch)
            salva_riusciti(rows_ok)
            # tolgo dagli errori originali (.MI) quelli risolti con .DE
            risolti_de = {t[:-3] + ".MI" for _, t, *_ in rows_ok}
            errors = [e for e in errors if e not in risolti_de]
